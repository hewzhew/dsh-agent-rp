    ...
    async character => {
      ...
      const decision = await runStage(input, 'character', generateOptions(
        ...
        [
          ...
          '不要写完整正文或逐字对白。只返回 JSON：{"observation":"此人能观察到的事实","action":"此人对刚发生结果的非规则反应，或空字符串","speech":{"respondsTo":"已公开的具体前提","move":"十一种动作之一","focus":"一个对象、区别或直接答案","effect":"希望对方当场改变的一项理解、决定或行动"},"insights":[{"kind":"knowledge|intention|decision|world-action","text":"knowledge 的私有事实、intention/decision 的当轮依据或待丢弃的规则动作","futureChoice":"intention/decision 唯一需要长期保存的原子选择，其他类型为空字符串"}]}。不开口时把 speech 设为 null。observation、action、speech 内容和 insights 不能包含引号包围的台词。不要使用 Markdown 围栏。',
        ].join('\n'),
        [
          context.text,

...
Agent RP 在 Session 恢复前向 DSH 注册自己声明的全部事件类型；这些事件默认必须可解释，缺少所有者插件时恢复会明确失败，不能用 `ignorable` 绕过。旧日志中的 Agent RP `ignorable` 标记只由修复命令移除，新写入不再产生该标记。`command/run` 与 `command/done` 只保存由玩家界面发起的命令生命周期，自动回合计划、后台模型收据和状态结算不能伪装成玩家命令。插件侧独立文件或数据库也不能替代这些事件：它无法与 Session 事件原子提交，并且不会自动进入原生分支与会话导出。

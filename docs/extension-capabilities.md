...
Agent RP 的私有 Session 事件通过 Host 的 `appendIgnorable()` 写入，并始终携带 `ignorable: true`。安装 Agent RP 时这些记录会恢复角色状态、回合计划和模型收据；未安装插件的 Host 可以跳过它们并继续读取同一 Session。对话表面事件仍必须由 DSH 解释，不能写成可跳过记录。`0.1.2-alpha.1` 迁移期生成过缺少该标记的 Agent RP 事件，修复命令只为这些记录补充标记。`command/run` 与 `command/done` 只保存由玩家界面发起的命令生命周期，自动回合计划、后台模型收据和状态结算不能伪装成玩家命令。插件侧独立文件或数据库也不能替代这些事件：它无法与 Session 事件原子提交，并且不会自动进入原生分支与会话导出。

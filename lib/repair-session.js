...
const AGENT_RP_SESSION_EVENT_TYPES = [
	...
	"agent-rp/story-turn-start",
	"agent-rp/story-turn-stopped",
	...
	while (start < input.length) {
		...
		if (typeof type === "string" && LEGACY_AGENT_RP_EVENT_TYPES.has(type)) {
			...
			if (record.ignorable === void 0) {
				...
				if (record.ignorable !== true) throw new Error(`事件 ${JSON.stringify(type)} 带有非法的 ignorable 标记`);
				alreadySafeEvents += 1;
...
try {
	...
	if (!apply) {
		...
		console.log(`缺少 ignorable 标记的 Agent RP 事件：${result.repairedEvents}`);
		console.log(`已经可安全跳过的 Agent RP 事件：${result.alreadySafeEvents}`);
		if (result.repairedEvents > 0) console.log("请先完全关闭 DSH，再用同一条命令加 --apply 执行。");
	} else if (result.applied) {
		console.log(`已为 ${result.repairedEvents} 条 Agent RP 事件补充 ignorable 标记。`);
